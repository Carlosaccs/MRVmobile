...
// 2. CARREGAMENTO DE DADOS (v140.4 - BUSCA DINÂMICA)
async function carregarPlanilha() {
    console.log("🔄 Iniciando leitura forçada dos 42 registros...");
    try {
        ...
        // Detecta se a planilha usa , ou ;
        const separador = linhas[0].includes(';') ? ';' : ',';
        window.bancoDados = {}; 

        linhas.slice(1).forEach((linha) => {
            // Divide a linha de forma simples para evitar erros de Regex
            const colunas = linha.split(separador).map(c => c.replace(/"/g, '').trim());

            // IMPORTANTE: Na sua planilha, o ID está na Coluna A (index 0)
            // e o Nome Curto (ZO Vivenza, etc) está na Coluna D (index 3)
            const idPlanilha = colunas[0] ? colunas[0].toLowerCase() : null;
            const nomeCurto = colunas[3]; 

            if (idPlanilha && nomeCurto) {
                window.bancoDados[idPlanilha] = {
                    nomeCurto: nomeCurto,
                    nomeFull: colunas[4] || nomeCurto,
                    estoque: colunas[5] || "0",
        ...
        const totalGeral = Object.keys(window.bancoDados).length;
        console.log("📊 Registros carregados no sistema:", totalGeral);

        // Força a atualização do menu e do mapa
        atualizarVisualizacao();
        popularMenuResidenciais();

    } catch (e) {
        console.error("❌ Falha na requisição da planilha:", e);
    }
}
