/* ==========================================================================
   v140.12 - DASHBOARD MOBILE: FIX TOTAL (MAPA, CLIQUES E COLUNA R)
   ========================================================================== */
        ...
        linhas.slice(1).forEach((linha) => {
            ...
            if (c.length >= 4) {
                ...
                window.dadosGerais.push({
                    ...
                    textoColunaR: limpar(c[17]), // MAPEADO R (ÍNDICE 17)
                    regional: reg
                });
            }
        });
    ...
    dados.paths.forEach(pData => {
        ...
        if (!ehMinimizado) {
            ...
            path.onclick = (e) => { 
                ...
                if (!ehMRV) return; // TRAVA DE CLIQUE NO CINZA
...
// 7. Event Listeners (Garantem o funcionamento dos botões)
window.onload = carregarPlanilha;
document.addEventListener('fullscreenchange', atualizarIconeFullscreen);
document.addEventListener('click', (e) => {
    if (e.target.closest('#btn-menu')) { toggleMenu(); }
    else if (e.target.closest('#btn-fullscreen')) { toggleFullscreen(); }
    else if (e.target.closest('#mapa-minimizado')) { trocarMapas(); }
});
