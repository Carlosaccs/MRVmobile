...
// 1. CARREGAMENTO COM CATEGORIA (COLUNA B)
async function carregarPlanilha() {
    try {
        ...
        if (total >= 42 && contador) {
            contador.innerText = total.toString().padStart(2, '0');
            contador.style.color = "#ADFF2F";
        }
...
// 2. MENU COM LOGICA DE ZONAS E CORES
function popularMenuResidenciais() {
    ...
    ordenados.forEach(item => {
        ...
        // Limpa o estoque se for apenas um traço
        const estoqueFinal = (item.estoque === "-" || item.estoque === "0") ? "" : item.estoque;

        card.innerHTML = `
            <span>${nomeUpper}</span>
            <span class="estoque-status">${estoqueFinal}</span>
        `;
        ...
        card.onclick = (e) => {
            ...
            if (pathOriginal) {
                ...
                // Não fechamos o menu aqui para o usuário ver o que clicou, 
                // mas se você preferir que feche, basta descomentar a linha abaixo:
                // toggleMenuLateral();
            }
        };
