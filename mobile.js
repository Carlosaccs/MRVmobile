...
// 2. CARREGAMENTO DE DADOS (v140.3 - DETECTOR DE SEPARADOR)
async function carregarPlanilha() {
    console.log("🔄 Tentando carregar os 42 registros...");
    try {
        const res = await fetch(URL_PLANILHA);
        const csv = await res.text();
        
        // 1. Limpa as linhas vazias
        const linhas = csv.split(/\r?\n/).filter(l => l.trim() !== "");
        if (linhas.length <= 1) {
            console.error("⚠️ Planilha vazia ou link incorreto.");
            return;
        }

        // 2. DESCOBRE O SEPARADOR (Vírgula ou Ponto e Vírgula)
        ...
        linhas.slice(1).forEach((linha, index) => {
            ...
            const c = linha.split(separador).map(item => item.replace(/"/g, '').trim());
            
            // c[0] costuma ser o ID e c[3] o Nome Curto (Coluna D)
            if (c.length >= 4) {
                const idOriginal = c[0];
                const idLimpo = idOriginal.toLowerCase();
                const nomeCurto = c[3];

                if (idLimpo && nomeCurto) {
                    window.bancoDados[idLimpo] = {
                        nomeCurto: nomeCurto,
                        nomeFull: c[4] || nomeCurto,
                        estoque: c[5] || "0",
                        statusObra: c[11] || "Consulte"
                    };
                }
            }
        });

        const totalEncontrado = Object.keys(window.bancoDados).length;
        console.log("✅ Total no Banco:", totalEncontrado);

        // Atualiza a tela e o menu
