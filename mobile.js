/* ==========================================================================
   v140.24 - DASHBOARD MOBILE: VERSÃO CONSOLIDADA E REPARADA
   ========================================================================== */
...
async function carregarPlanilha() {
    try {
        ...
        linhas.slice(1).forEach((linha) => {
            // Tratamento para vírgulas dentro de aspas (CSV seguro)
            ...
            if (c.length >= 15) { 
                ...
                window.dadosGerais.push({
                    ...
                    nomeCurto: reg ? `${limpar(c[3])} - ${reg}` : limpar(c[3]),
                    estoque: limpar(c[5]),
                    endereco: limpar(c[6]),
                    entrega: limpar(c[8]),
    ...
    dados.paths.forEach(pData => {
        ...
        if (!ehMinimizado) {
            path.onmouseover = () => { 
                ...
                if (path.getAttribute('data-selecionado') !== 'true') path.style.fill = ehMRV ? corLaranja : corHoverCinza; 
            };
            path.onmouseout = () => { 
...
// BLOCO 6: EXIBIÇÃO DE DETALHES (CORREÇÃO BOTÕES)
    ...
    if(elDetalhes) {
        if (info.categoria === "COMPLEXO") {
            elDetalhes.innerHTML = `
                ...
                <div id="texto-descricao" class="texto-complexo-estilo">${info.textoColunaR || ""}</div>
            `;
            return;
        }

        const linkMaps = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(info.endereco)}`;
        let valorObra = (info.obra || "0").includes('%') ? info.obra : (info.obra || "0") + "%";
        ...
        } else if (valorEstoque !== "" && valorEstoque !== "VAZIO") {
            txtEstoque = `RESTAM ${valorEstoque} UN.`;
            if (parseInt(valorEstoque) < 6) estiloEstoque = "color: #e31c19;"; 
        }
        ...
        const cssValor = `font-weight: 800; font-size: 11px; text-align: right; color: #333;`;
        ...
        elDetalhes.innerHTML = `
            ...
            <div class="container-acoes">
                ...
                <div style="display: flex; gap: 8px; margin-top: 8px; justify-content: flex-start;">
                    ...
                    <button onclick="copyToClipboard('${info.link}')" class="btn-acao btn-link" style="flex: 0 1 auto; min-width: 80px;">LINK</button>
                </div>
            </div>

            <div class="grid-caixas-mobile" style="display: flex; flex-wrap: wrap; gap: 4px; margin-top: 10px; width: 100%;">
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">ENTREGA</span><span style="${cssValor}">${info.entrega || "-"}</span></div>
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">OBRA</span><span style="${cssValor}">${valorObra}</span></div>
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">PLANTAS</span><span style="${cssValor}">${info.plantaMin} a ${info.plantaMax}m²</span></div>
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">ESTOQUE</span><span style="${cssValor}${estiloEstoque}">${txtEstoque}</span></div>
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">LIMITADOR</span><span style="${cssValor}">${info.limitador || "-"}</span></div>
                <div class="caixa-dado" style="${cssCaixa}"><span style="${cssLabel}">C. PAULISTA</span><span style="${cssValor}">${info.cPaulista || "NÃO"}</span></div>
            </div>
