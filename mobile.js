/* ==========================================================================
   v150 - INTEGRAL: CONEXÃO PLANILHA + ÍCONES INKSCAPE + MOBILE FIX
   ========================================================================== */
    ...
    dados.paths.forEach(pData => {
        ...
        if (!ehMinimizado) {
            path.onclick = () => {
                ...
                if (info) {
                    document.getElementById('nome-imovel').innerText = info.nomeCurto || "Residencial";
                    document.getElementById('detalhes-imovel').innerHTML = `
                        <p style="margin-bottom:10px;"><strong>Unidades:</strong> ${info.estoque || 'Sob Consulta'}</p>
                        <p><strong>Status:</strong> ${info.statusObra || 'Lançamento'}</p>
                    `;
                }
            };
        }
