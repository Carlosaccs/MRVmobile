/* ==========================================================================
   v140.2 - CORREÇÃO FULLSCREEN + INVERSÃO DE POSIÇÃO (MENU NO TOPO)
   ========================================================================== */
...
/* ==========================================================================
   MENU E UTILITÁRIOS (TOPO E RODAPÉ)
   ========================================================================== */
...
// CORREÇÃO E CONTROLE DE TELA CHEIA
...
function atualizarIconeFullscreen() {
    ...
    const svg = p ? p.closest('svg') : null;
    if (!p || !svg) return;
...
function toggleMenu() {
    ...
    if(menu) { menu.classList.toggle('menu-oculto'); menu.classList.toggle('menu-aberto'); }
}

// Inicialização e Listeners
window.onload = carregarPlanilha;
document.addEventListener('fullscreenchange', atualizarIconeFullscreen);
document.addEventListener('webkitfullscreenchange', atualizarIconeFullscreen); // Safari

document.addEventListener('click', (e) => {
    // Clique no mapa minimizado
    if (e.target.closest('#mapa-minimizado')) trocarMapas();
    
    // Clique no botão de Menu (agora em cima)
    const btnMenu = e.target.closest('.btn-toggle-menu') || e.target.closest('#btn-menu');
    if (btnMenu) toggleMenu();

    // Clique no botão de Fullscreen (agora embaixo)
    const btnFull = e.target.closest('.btn-fullscreen') || e.target.closest('#btn-fullscreen');
