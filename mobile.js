/* ==========================================================================
   v140.26 - RETROCESSO ESTÁVEL: MAPAS + MENU + COLUNA P
   ========================================================================== */
...
// DNAs dos ícones (Mantidos da sua versão original)
...
function exibirDadosResidencial(info) {
    ...
    if(elDetalhes) {
        ...
        if (info.destaqueVermelho && info.destaqueVermelho !== "") {
            htmlDestaque = `<div style="background:#fff5f5; border:1px solid #ffe3e3; color:#e31c19; font-weight:800; text-align:center; padding:4px; margin:8px 0; border-radius:4px; font-size:10px;">${info.destaqueVermelho.toUpperCase()}</div>`;
        }
        ...
        elDetalhes.innerHTML = `
            ...
            <div class="grid-caixas-mobile" style="display:flex; flex-wrap:wrap; gap:4px; margin-top:5px; width:100%;">
                ...
                <div style="${cssCaixa}"><span style="color:#00713a;font-weight:bold;font-size:10px;">OBRA</span><span style="font-weight:800;font-size:11px;">${info.obra}%</span></div>
                <div style="${cssCaixa}"><span style="color:#00713a;font-weight:bold;font-size:10px;">PLANTAS</span><span style="font-weight:800;font-size:11px;">${info.plantaMin}m²</span></div>
                <div style="${cssCaixa}"><span style="color:#00713a;font-weight:bold;font-size:10px;">ESTOQUE</span><span style="font-weight:800;font-size:11px;">${info.estoque}</span></div>
            </div>
            <div style="color:#ffffff; margin-top:10px; font-size: 11px; text-align:justify; white-space: pre-line;">${info.textoColunaR || ""}</div>
        `;
    }
}
...
function gerarMenuResidenciais() {
    const containerMenu = document.getElementById('lista-residenciais'); // ID conforme seu HTML
    if (!containerMenu) return;

    containerMenu.innerHTML = "";
    const filtrados = window.dadosGerais.filter(d => 
        d.id && (d.regional === (mapaAtivo === "GSP" ? "GSP" : "INTERIOR"))
    );
...
function toggleMenu() {
    ...
    if(menu) menu.classList.toggle('menu-oculto');
}
